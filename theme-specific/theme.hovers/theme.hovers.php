...
if (!function_exists('chocorocco_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'chocorocco_hovers_theme_setup3', 3 );
	function chocorocco_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		chocorocco_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'chocorocco'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'chocorocco') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'chocorocco')
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'chocorocco'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'chocorocco') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'chocorocco'),
					'icon'	=> esc_html__('Icon',	'chocorocco'),
					'icons'	=> esc_html__('Icons',	'chocorocco'),
					'zoom'	=> esc_html__('Zoom',	'chocorocco'),
					'fade'	=> esc_html__('Fade',	'chocorocco'),
					'slide'	=> esc_html__('Slide',	'chocorocco'),
					'pull'	=> esc_html__('Pull',	'chocorocco'),
					'border'=> esc_html__('Border',	'chocorocco')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('chocorocco_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'chocorocco_hovers_theme_setup9', 9 );
	function chocorocco_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'chocorocco_hovers_frontend_scripts', 1010 );
		add_filter( 'chocorocco_filter_localize_script','chocorocco_hovers_localize_script' );
		add_filter( 'chocorocco_filter_merge_scripts',	'chocorocco_hovers_merge_scripts' );
		add_filter( 'chocorocco_filter_merge_styles',	'chocorocco_hovers_merge_styles' );
		add_filter( 'chocorocco_filter_get_css', 		'chocorocco_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'chocorocco_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'chocorocco_hovers_frontend_scripts', 1010 );
	function chocorocco_hovers_frontend_scripts() {
		if ( chocorocco_is_on(chocorocco_get_theme_option('debug_mode')) && chocorocco_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'chocorocco-hovers', chocorocco_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( chocorocco_is_on(chocorocco_get_theme_option('debug_mode')) && chocorocco_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'chocorocco-hovers',  chocorocco_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('chocorocco_hovers_merge_scripts')) {
	//Handler of the add_filter( 'chocorocco_filter_merge_scripts', 'chocorocco_hovers_merge_scripts' );
	function chocorocco_hovers_merge_scripts($list) {
...
if (!function_exists('chocorocco_hovers_merge_styles')) {
	//Handler of the add_filter( 'chocorocco_filter_merge_styles', 'chocorocco_hovers_merge_styles' );
	function chocorocco_hovers_merge_styles($list) {
...
if (!function_exists('chocorocco_hovers_localize_script')) {
	//Handler of the add_filter( 'chocorocco_filter_localize_script','chocorocco_hovers_localize_script' );
	function chocorocco_hovers_localize_script($arr) {
		$arr['button_hover'] = chocorocco_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('chocorocco_hovers_add_icons') ) {
	function chocorocco_hovers_add_icons($hover, $args=array()) {
...
// Add styles into CSS
if ( !function_exists( 'chocorocco_hovers_get_css' ) ) {
	//Handler of the add_filter( 'chocorocco_filter_get_css', 'chocorocco_hovers_get_css', 10, 4 );
	function chocorocco_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
/* Slide */
.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['bd_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
