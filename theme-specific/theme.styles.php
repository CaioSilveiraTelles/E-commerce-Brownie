...
if (!function_exists('chocorocco_customizer_get_css')) {

	function chocorocco_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = chocorocco_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('chocorocco_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
CSS;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(chocorocco_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = chocorocco_get_scheme_colors($scheme);
...
.chocorocco_tabs .chocorocco_tabs_titles li a:hover {
	color: {$colors['alter_dark']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.chocorocco_tabs .chocorocco_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text']};
}
...
CSS;
					...
					$rez = apply_filters('chocorocco_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'chocorocco_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>