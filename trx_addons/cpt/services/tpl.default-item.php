...
?>">
	...
	if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']=='image')) {
		...
		?><a href="<?php echo esc_url($link); ?>"
			...
			if (!empty($svg)) {
				...
				?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php echo esc_html(basename($img)); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			}
		?></a><?php
	} else {
	...
	<div class="sc_services_item_info">
		...
		<?php } ?>
        <div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="sc_button sc_button_simple"><?php esc_html_e('Details', 'chocorocco'); ?></a></div>
